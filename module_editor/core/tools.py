...
class DrawingTool:
    @staticmethod
    def render_qt(painter, v_type, coords, color, width):
        x1, y1, x2, y2 = coords
    ...
    @staticmethod
    def render_pil(draw, v_type, coords, color, width):
        x1, y1, x2, y2 = coords
        
        if v_type == "rect":
            DrawingTool._pil_draw_round_line(draw, [(x1, y1), (x2, y1), (x2, y2), (x1, y2), (x1, y1)], color, width)
        elif v_type == "arrow":
            dx, dy = x2 - x1, y2 - y1
            arrow_wing_len = constants.VECTOR_STYLE["arrow_wing_len"]
            if math.hypot(dx, dy) < max(arrow_wing_len * 0.35, width * 2):
                return
            DrawingTool._pil_draw_round_line(draw, [(x1, y1), (x2, y2)], color, width)
            ang = math.atan2(dy, dx)
            wlen = arrow_wing_len
            for a in [-math.pi / 6, math.pi / 6]:
                wx, wy = x2 - wlen * math.cos(ang - a), y2 - wlen * math.sin(ang - a)
                DrawingTool._pil_draw_round_line(draw, [(x2, y2), (wx, wy)], color, width)
        elif v_type == "highlighter":
