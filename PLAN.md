...
1. `config.py`: Módulo para salvar y leer propiedades de un `config.json`. Ahora deduce dinámicamente si el SO usa "Documentos" o "Documents" a través de `winreg` usando su propia variable de entorno base para conformar el _default_ dir de `QAScreens`.
